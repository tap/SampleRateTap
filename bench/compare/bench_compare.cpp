...
public:
    InputTap(std::size_t frames, std::size_t channels)
        : buf_(sineInput(frames, channels)), frames_(frames), ch_(channels) {}
    ...
    std::vector<S> buf(48000 * channels);
    {
        ...
        for (std::size_t i = 0; i < tmp.size(); ++i) {
            if constexpr (std::is_floating_point_v<S>)
                ...
                buf[i] =
                    srt::detail::roundSat<S>(static_cast<double>(tmp[i]) *
...
void soxrBench(benchmark::State& state, unsigned long recipe, std::size_t channels) {
    ...
    soxr_t soxr = soxr_create(48000.0, 48000.0 * kRatio, static_cast<unsigned>(channels), &err, &io,
                              &q, nullptr);
    if (err != nullptr) {
    ...
    state.counters["latency_frames"] =
        benchmark::Counter(soxr_delay(soxr), benchmark::Counter::kAvgThreads);
    soxr_delete(soxr);
    state.SetItemsProcessed(frames);
}
