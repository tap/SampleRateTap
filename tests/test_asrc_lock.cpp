...
TEST(AsrcLock, TracksDriftRampWithoutUnlocking) {
    srt::AsyncSampleRateConverter asrc(monoConfig());
    srt_test::TwoClockSim sim{
        .asrc = asrc, .fsIn = kFs, .fsOut = kFs, .channels = 1, .chunkIn = 1, .chunkOut = 1};
...
TEST(AsrcLock, WholeSampleSlipsAreGlitchFree) {
    ...
    sim.gen = [&](std::uint64_t i) {
        return static_cast<float>(amp *
                                  std::sin(2.0 * std::numbers::pi * nu * static_cast<double>(i)));
    };
    ...
    for (std::size_t n = 1; n + 1 < tail.size(); ++n) {
        const double d2 = std::abs(static_cast<double>(tail[n + 1]) - 2.0 * tail[n] +
                                   static_cast<double>(tail[n - 1]));
...
TEST(AsrcLock, RecoversFromConsumerStall) {
    ...
    srt::AsyncSampleRateConverter asrc(monoConfig());
    srt_test::TwoClockSim sim{
        .asrc = asrc, .fsIn = kFs * (1.0 + 100e-6), .fsOut = kFs, .channels = 1};
