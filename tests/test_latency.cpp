...
TEST(Latency, ImpulseDelayMatchesDesignedLatency) {
    ...
    out.reserve(60000);
    sim.run(1.2, [&](const float* x, std::size_t frames, double) {
        out.insert(out.end(), x, x + frames);
    });
    ...
    const double measuredDelay =
        static_cast<double>(peak) + refine - static_cast<double>(impulseIndex);
...
TEST(Latency, DesignedLatencyConsistency) {
    ...
    EXPECT_NEAR(asrc.designedLatencySeconds(),
                (static_cast<double>(cfg.targetLatencyFrames) + groupDelay) / kFs, 1e-12);
