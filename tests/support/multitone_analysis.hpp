...
namespace srt_test {
    ...
    struct ToneComb {
        ...
        static ToneComb pink(std::size_t k, double fLo, double fHi, double peakSum) {
            ...
            for (std::size_t i = 0; i < k; ++i) {
                const double f = fLo * std::pow(fHi / fLo, static_cast<double>(i) / static_cast<double>(k - 1));
        ...
        double sampleAt(std::uint64_t i, double fs) const {
            ...
            for (std::size_t k = 0; k < freqHz.size(); ++k)
                v += amplitude[k]
                     * std::sin(2.0 * std::numbers::pi * freqHz[k] / fs * static_cast<double>(i) + phase[k]);
            return v;
        }
    };
    // ANCHOR_END: pw_comb
    ...
    inline double programWeightedSnrDb(std::span<const float> tail, const ToneComb& comb, double /*fsIn*/,
                                       double fsOut) {
