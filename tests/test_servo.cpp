...
namespace {
    ...
    constexpr double      k_k_target = 48.0;
    constexpr std::size_t k_k_block  = 32;
    constexpr double      k_k_dt     = static_cast<double>(k_k_block) / k_k_fs;

    // Pure plant simulation: the FIFO integrates the rate mismatch.
    struct plant {
        double occ = k_k_target;
        void   step(double eps_true, double eps_hat) { occ += (eps_true - eps_hat) * k_k_fs * k_k_dt; }
    };
    ...
    TEST(Servo, LocksFromConstantOffsetAndNullsError) {
        ...
        EXPECT_NEAR(plant.occ, k_k_target, 0.05);
    ...
    TEST(Servo, BandwidthSwitchIsTransientFree) {
        ...
        double max_err = 0.0;
        for (double s = 0.0; s < 10.0; s += k_k_dt) {
            plant.step(eps_true, servo.update(plant.occ, 0.0, k_k_dt));
            max_err = std::max(max_err, std::abs(plant.occ - k_k_target));
        }
    ...
    TEST(Servo, ClampsToMaxDeviation) {
        ...
        srt::pi_servo servo(cfg, k_k_fs, k_k_target);
        // Huge occupancy error must saturate at 1.5x the configured range.
        const double eps = servo.update(k_k_target + 10000.0, 0.0, k_k_dt);
