...
namespace {
    ...
    double measureProgramSnrDb(const srt::FilterSpec& spec) {
        ...
        srt_test::TwoClockSim sim{.asrc = asrc, .fsIn = fsIn, .fsOut = kFs, .channels = 1, .chunkIn = 1, .chunkOut = 1};
    ...
    double measureSineSnrDb(const srt::FilterSpec& spec, double freqHz) {
        ...
        sim.gen           = [&](std::uint64_t i) {
            return static_cast<float>(0.5 * std::sin(2.0 * std::numbers::pi * nuIn * static_cast<double>(i)));
        };
        std::vector<float> tail;
    ...
    TEST(ProgramWeighted, InstrumentFloor) {
        ...
        for (std::size_t i = 0; i < tail.size(); ++i) {
            ...
            for (std::size_t k = 0; k < comb.freqHz.size(); ++k)
                v += comb.amplitude[k]
                     * std::sin(2.0 * std::numbers::pi * comb.freqHz[k] * rho / kFs * static_cast<double>(i)
                                + comb.phase[k]);
