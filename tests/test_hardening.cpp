...
namespace {
    ...
    TEST(Reset, ConsumerResetRelocks) {
        ...
        srt_test::TwoClockSim sim2{.asrc = asrc, .fsIn = kFs * (1.0 + 200e-6), .fsOut = kFs, .channels = 1};
    ...
    TEST(QuickQuality, Q15Tone997) {
        ...
        srt_test::TwoClockSimT<std::int16_t> sim{
            .asrc = asrc, .fsIn = kFs * (1.0 + 200e-6), .fsOut = kFs, .channels = 1, .chunkIn = 8, .chunkOut = 8};
    ...
    TEST(QuickQuality, FullScaleQ15Short) {
        ...
        srt::AsyncSampleRateConverterQ15     asrc(cfg);
        srt_test::TwoClockSimT<std::int16_t> sim{
            .asrc = asrc, .fsIn = kFs * (1.0 + 500e-6), .fsOut = kFs, .channels = 1, .chunkIn = 8, .chunkOut = 8};
