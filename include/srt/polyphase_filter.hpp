...
namespace srt {
    ...
    class PolyphaseFilterBank {
      public:
        ...
        PolyphaseFilterBank(const FilterSpec& spec, double sampleRateHz)
            : phases_(std::bit_ceil(spec.numPhases))
            , taps_(spec.tapsPerPhase) {
            ...
            if (spec.passbandHz <= 0.0 || spec.stopbandHz <= spec.passbandHz || spec.stopbandHz > sampleRateHz)
            ...
            if (spec.imageZeros)
                detail::designPrototypeCompensated(proto, phases_, cutoffNorm, detail::kaiserBeta(spec.stopbandAttenDb),
                                                   spec.passbandHz / sampleRateHz);
            else
                detail::designPrototype(proto, phases_, cutoffNorm, detail::kaiserBeta(spec.stopbandAttenDb));
    ...
    /// every channel.
    template <SampleType S>
    inline void blendRow(const PolyphaseFilterBank<S>& bank, typename SampleTraits<S>::Coeff* SRT_RESTRICT row,
                         double mu) noexcept {
        using Tr         = SampleTraits<S>;
        const double pos = mu * static_cast<double>(bank.numPhases());
        std::size_t  p   = static_cast<std::size_t>(pos);
    ...
    template <SampleType S>
    inline void blendRowPhase(const PolyphaseFilterBank<S>& bank, typename SampleTraits<S>::Coeff* SRT_RESTRICT row,
    ...
    template <SampleType S>
    inline S interpolatePhase(const PolyphaseFilterBank<S>& bank, const S* hist, std::uint64_t phase) noexcept {
        using Tr              = SampleTraits<S>;
    ...
    inline void dotTileFrameMajor(const typename SampleTraits<S>::Coeff* SRT_RESTRICT row, const S* SRT_RESTRICT x,
    ...
    template <SampleType S>
    inline void dotRowsFrameMajor(const typename SampleTraits<S>::Coeff* SRT_RESTRICT row, const S* SRT_RESTRICT x,
    ...
    class FractionalResampler {
      ...
      public:
        ...
        static constexpr bool kChannelParallel = SRT_CHANNEL_PARALLEL != 0 && std::is_floating_point_v<S>;

        /// Allocates histories and the pop scratch buffer; setup time only.
        FractionalResampler(const PolyphaseFilterBank<S>& bank, std::size_t channels, std::size_t chunkFrames = 64)
            : bank_(&bank)
            , channels_(channels)
            , chunk_(chunkFrames)
            , histCap_(bank.taps() + chunkFrames)
            , scratch_(chunkFrames * channels)
            , frameMajor_(kChannelParallel && channels >= SRT_CP_MIN_CHANNELS)
            , hist_(frameMajor_ ? 1 : channels)
            , row_(bank.taps()) {
