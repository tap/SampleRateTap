...
```cpp
blend_row(bank, row, mu);          // once per frame
for (std::size_t c = 0; c < channels; ++c)
    out[c] = dot_row(row, window(c), taps);
```
...
**Finding 1: `blend_row` vectorized, but behind a runtime aliasing check.**
...
check from `blend_row`*. The fixed-point pipelines blend through the same
