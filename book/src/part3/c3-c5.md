...
Q15 MACs: precisely the inner operation of `dot_row`, at double width.
...
`blend_row` + `dot_row` rather than the fused `interpolate_phase()`, because
the dual-MAC loop lives in `dot_row` — legitimate only because C1
