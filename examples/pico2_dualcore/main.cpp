...
namespace {
    ...
    PhaseResult runPhase(const PhaseSpec& ph) {
        ...
        std::printf("PHASE %c %s: %lu s run, lock limit %.1f s, ppm gate +/-%.0f after %.0f s\n", ph.tag, ph.desc,
                    static_cast<unsigned long>(ph.runMs / 1000), static_cast<double>(ph.lockLimitMs) / 1000.0,
                    kPpmTolerance, static_cast<double>(ph.ppmSettleMs) / 1000.0);
...
int main() {
    ...
    std::printf("sys clock %lu Hz; core0 push @ nominal*(1+%.0fe-6), core1 pull @ nominal; "
                "block %u frames\n",
                static_cast<unsigned long>(clock_get_hz(clk_sys)), kOffsetPpm, static_cast<unsigned>(kBlockFrames));
    ...
    const PhaseSpec phases[] = {
        {'A', "q15 2ch balanced @48000", 2, 48000.0, false, 10000000, 15003, 2000, 3, 7500, 2000, 10000, 30000},
        {'B', "q15 12ch balanced16k @16000", 12, 16000.0, true, 10000000, 5001, 2000, 1, 2500, 6000, 15000, 30000},
    };
