...
namespace {
    ...
    constexpr double      k_k_consumer_ppm = 500.0; // consumer clock runs 500 ppm fast
    constexpr std::size_t k_k_chunk        = 96;
    constexpr double      k_k_run_seconds  = 20.0;
...
int main() {
    ...
    std::thread producer([&] {
        std::vector<float> buf(k_k_chunk);
    ...
    std::thread consumer([&] {
        std::vector<float> buf(k_k_chunk);
        const double       fs_out = k_k_fs * (1.0 + k_k_consumer_ppm * 1e-6);
        auto               next   = t0;
        const auto         period = std::chrono::duration_cast<std::chrono::steady_clock::duration>(
            std::chrono::duration<double>(static_cast<double>(k_k_chunk) / fs_out));
        while (!stop.load(std::memory_order_relaxed)) {
            asrc.pull(buf.data(), k_k_chunk);
            next += period;
            std::this_thread::sleep_until(next);
        }
    });
    ...
    for (double t = 0.5; t <= k_k_run_seconds; t += 0.5) {
    ...
    std::printf("\nfinal: state=%s ppm(avg)=%+.2f (expected ~%+.0f)\n", state_name(st.state), ppm_avg,
                -k_k_consumer_ppm);
    const bool ok = st.state == srt::converter_state::locked && std::abs(ppm_avg + k_k_consumer_ppm) < 150.0;
